...
6.empty() : biến ko tồn tại trả về true;
    6.1 6// 6 trường hợp trả về true nếu biến mang giá trị : 0 - "0" , NULL ,"", FALSE.
            //và biến không tồn tại
                              ...
                              // các loại dữ liệu trong PHP ( 8 kiểu )
...
8. resource // nơi lưu trữ tài nguyên bên ngoài
                                ...
                                1.//  STRING : chuỗi
1.1 strlen(); //đếm chuỗi;
1.2 str_word_count(); // đếm từ trong chuỗi;
1.3 strrev(); // đảo ngược chuỗi;
1.4 strpos(" văn bản","đoạn văn bản muốn tìm") //; tìm vị trí xuất hiện đầu tiên;
1.5.str_replace("từ bị thay","từ muốn thay","văn bản gốc");
1.6 lọc chuỗi : $lochuoi = filter_var($string, FILTER_SANITIZE_STRING);
1.7 (string)$bien : //ép dữ liệu sang kiểu chuỗi ;


                               2.// INTERGET(SỐ NGUYÊN ) - INT
2.1 LỌC int : if(filter_var($int, FILTER_VALIDATE_INT)){
    echo " là số nguyên";
}
2.2(int)$bien : //ép dữ liệu sang kiểu int;

                               3.//FLOAT - (kiểu số thực, là dạng số thập phân hay dạng lũy thừa),
3.1. (float)$bien : // gán dữ liệu thành kiểu float;
...
4.2 làm việc với array :
  ...
  4.2b - lấy độ dài : count($ten_array);
  4.2c - xóa phần tử : unset($array[0]);
  4.2d - sắp xếp array:
                - tăng dần : sort();
                - giảm dần : rsort();
  ...
  4.2.f - đổi dạng  array-> string : implode("ký tự phân cách".$bien_array);
        vd: $a = [18,11,1999];
            $b = implode("/",$a);
...
1. date("d/m/Y");
 1.1 giờ : date("h:i:sa"); // H in hoa 24h , h 12h;
                ...
                // X - COOKIE
- Cookie là một tệp nhỏ dùng để lưu trữ dữ liệu trên Client;
-lưu trữ các tùy chọn riêng của trang web từng user
- thường được sử dụng để xác định người dùng
    $cookie_name = "loc";
...
1. setcookie($cookie_name, $value_cookie,time()+(86400),"/");
 ...
 1.2 sửa cookie :  sửa trên biến;
                ...
                // XI - SESSON
-Session dùng để lưu trữ dữ liệu trên Server
- được dùng để lưu trữ thông tin của người dùng hoặc
    lưu trữ tùy chọn cấu hình hệ thống cho người dùng
